\newcommand{\prizvuk}[1]{%
{\textbf{#1}}%
}
\newcommand{\priprava}[1]{%
{\textit{#1}}%
}
\newcommand{\flexa}[1]{%
{\underline{#1}}%
}
\newcommand{\nadpisa}[1]{%
{\begin{center}\large{\textbf{\textsc{#1}}}\end{center}}%
}
\newcommand{\nadpisb}[1]{%
{\begin{center}\large{\textsc{#1}}\end{center}}%
}
\newcommand{\nadpisc}[1]{%
{\textit{\small{\textbf{#1}}}}%
}
\newcommand{\nadpisd}[1]{%
{\begin{center}\textit{\small{#1}}\end{center}}%
}
\newcommand{\nadpisleft}[1]{%
{\color{red} \textit{\small{#1}}}%
\nopagebreak
\vspace{0.2em}%
\nopagebreak
}
\newcommand{\nadpisleftclose}[1]{%
{\color{red} \textit{\small{#1}}}%
\vspace{-1em}%
}
\newcommand{\nadpissub}[2]{%
{\begin{center}\textbf{\textsc{#1}}
\linebreak
\textit{\small{#2}}\end{center}}%
}
\newcommand{\nadpisf}[1]{%
{\begin{center}{\textsc{#1}}\end{center}}%
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\textaccent}[1]{%
{\textbf{{#1}}}%
%{\color{red} \textbf{\large{#1}}}%
}
\newcommand{\textprep}[1]{%
%{\color{red} \textit{\large{#1}}}%
{\textit{{#1}}}%
}
\newcommand{\textflex}[1]{%
{\underline{#1}}%
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\oratio}[1]{%
\nadpisleft{Oratio:}

\input{./other/domine_exaudi_orationem.tex}

%\vspace{0.5em}

\input{{#1}}

%\vspace{0.5em}

\input{./other/domine_exaudi_orationem.tex}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohschapter}[1]{%
\chapter*{{#1}}%
\thispagestyle{fancy}%
\def \ohschaptername {{#1}}
\grechangedim{spacelinestext}{6 mm}{scalable}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsannotation}[2]{%
\greannotation{\color{red} \small {#1}}%
\greannotation{\small {#2}}%
\gresetinitiallines{1}%
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsscorenostar}[1]{%
\gresetinitiallines{0}%
{\let\greheightstar\relax%
\gregorioscore{#1}%
}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\translateto{EN}
\def\versseparator{\ding{88}~}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohstranslate}[1]{%
\IfFileExists{./translations/\translateto/#1.tex}{
	\hrulefill
	\begin{footnotesize}
		\textit{\input{./translations/\translateto/#1.tex}}
		\hrulefill
		\par
	\end{footnotesize}
}
{
	{\color{red}
	\begin{footnotesize}
	\hrulefill
	\url{#1.tex}
	\hrulefill
	\end{footnotesize}
	}
}
\par
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohspsalmus}[3]{%
\gresetinitiallines{0}%
\nadpisleft{#1}
\gregorioscore{#2}
\vspace{-0.8em}
\input{#3}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohspsalmusnosep}[3]{%
\gresetinitiallines{0}%
\nadpisleft{#1}
\vspace{-0.6em}
\gregorioscore{#2}
\vspace{-1.4em}
\input{#3}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsresponsorium}[3]{%
\ohsformatresponsorium
\ohsannotation{Resp. #1}{#2}
\gregorioscore{./resp/#3.tex}
\ohstranslate{#3}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsresponsoriumnt}[3]{%
\ohsformatresponsorium
\ohsannotation{Resp. #1}{#2}
\gregorioscore{./resp/#3.tex}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsproprium}[3]{%
\ohsformatresponsorium
\gresetinitiallines{1}
\ohsannotation{#1}{#2}
\gregorioscore{./mass/#3.tex}
\ohstranslate{#3}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohspropriumcentered}[3]{%
\vspace*{\stretch{4}}
\ohsformatresponsorium
\gresetinitiallines{1}
\ohsannotation{#1}{#2}
\gregorioscore{./mass/#3.tex}
\ohstranslate{#3}
\separator{1}
\vspace*{\stretch{4}}
\pagebreak
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohsverses}[3]{%
\ohsformatverses
\gresetinitiallines{0}
\nadpisleft{#1}
\begin{small}
\gregorioscore{./mass/#2.tex}
\end{small}
\ohstranslate{#2}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ohslectio}[1]{%
\input{./lectio/#1.tex}
\ohstranslate{#1}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\redvbar}{%
\textbf{\color{red} \Vbar.}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\redrbar}{%
\textbf{\color{red} \Rbar.}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def \ohszlsc {
\GreNewParLine %
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
